...
export async function inviteCoach(
  ...
  try {
    ...
    if (!profile?.roles?.includes('admin') && !profile?.roles?.includes('super_admin')) {
      return { success: false, error: 'Unauthorized - only admins can invite coaches' }
    }
    ...
    if (existingUser) {
      // User exists - check if they're already a coach
      ...
      if (currentRoles.includes('coach')) {
        return { success: false, error: 'User is already a coach' }
      }

      const newRoles = [...currentRoles, 'coach']
      ...
      if (!updatedProfile?.roles?.includes('coach')) {
        console.error('Coach role update failed silently for user:', existingUser.id)
